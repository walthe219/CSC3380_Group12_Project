...
public class GunLifeSteal : MonoBehaviour
{
    ...
    [Min(0)]
    [SerializeField] float lifeAmountStolen = 0.1f;
    private void Start()
    {
    ...
    void gainHealth(RaycastHit NOTUSED)
    {
        ...
        {
            currPlayerStats.health = (float)(currPlayerStats.health + (currPlayerStats.damage * lifeAmountStolen)); //If LifeSteal Upgrade is unlocked, then whena player successfully
    ...
    void loseHealth(RaycastHit NOTUSED)
    {
        if (LifeStealUnlocked && currPlayerStats.health < BasePlayerStats.health)
        {
            currPlayerStats.health = (float)(currPlayerStats.health - (currPlayerStats.damage * lifeAmountStolen)); //Where the player loses the percentage of health if they miss
        }
    }
}
